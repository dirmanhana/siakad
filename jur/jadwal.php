...
function TampilkanHeaderJadwal() {
  ...
  $optkelas = GetOption2('kelas', 'Nama', 'Nama', $_SESSION['_jdwlKelas'], "ProdiID='$_SESSION[_jdwlProdi]' AND TahunID='$_SESSION[_jdwlTahun]' and KodeID='".KodeID."'", "KelasID");
  ...
  <tr><td class=wrn width=2 rowspan=5></td>
      ...
      <td class=ul1><select name='_jdwlTahun' onChange='this.form.submit()' />$opttahun</select></td>
      <td class=inp>Prg. Pendidikan:</td>
      <td class=ul1><select name='_jdwlProg'>$optprog</select></td>
      <td class=inp>Program Studi:</td>
      <td class=ul1 colspan=3><select name='_jdwlProdi' onChange='this.form.submit()'>$optprodi</select></td>
      </tr>
  <tr><td class=inp>Hari:</td>
      <td class=ul1><select name='_jdwlHari'>$opthr</select></td>
      <td class=inp>Kelas:</td>
      <td class=ul1><select name='_jdwlKelas' value='$_SESSION[_jdwlKelas]'>$optkelas</select></td>
...
function DftrKuliah() {
  ...
  while ($w = _fetch_array($r)) {
    ...
    if ($w['Final'] == 'Y') {
      ...
      $del = "<a href='#' onClick=\"javascript:JdwlDel($w[JadwalID])\" title='Hapus jadwal'><img src='img/del.gif' /></a>";
      $c = "class=ul";
      ...
      $print = "$kanan <a href='#' onClick=\"javascript:CetakDaftar($w[JadwalID])\">Daftar</a><br />
          $kanan <a href='#' onClick=\"javascript:CetakKursiUTS($w[JadwalID])\">Kursi UTS</a><br>
...
function AmbilResponsi($id) {
  ...
  while ($w = _fetch_array($r)) {
	...
	$n++;
	$a[] = "&rsaquo; <b>$w[_NamaJenisJadwal] #$n</b> $w[_NamaHari], $w[_JM] - $w[_JS], $w[_NamaRuang]($w[RuangID]) <a href='#' onClick=\"JdwlLabEdt(0, '$w[JadwalRefID]', '$w[JadwalID]')\"><img src='img/edit.png' /></a> <a href='#' onClick=\"JdwlLabDel('$w[JadwalID]')\"><img src='img/del.gif' /></a>";
  }
  $a = (!empty($a))? "<br />".implode("<br />", $a) : '';
  return $a;
}
...
function JdwlEdtScript() {
  ...
  function JdwlLabDel(id) {
    if (confirm("Anda yakin akan menghapus jadwal Lab/Responsi ini?")) {
      var _rnd = randomString();
      window.location = "?mnux=$_SESSION[mnux]&BypassMenu=1&gos=JdwlLabDel&id="+id+"&_rnd="+_rnd;
    }
  }
  ...
  function CetakJadwalRuang() {
      var _rnd = randomString();
      lnk = "$_SESSION[mnux].ruang.pdf.php?TahunID=$_SESSION[_jdwlTahun]&ProdiID=$_SESSION[_jdwlProdi]+_rnd="+_rnd;
      win2 = window.open(lnk, "", "width=700, height=500, scrollbars, status");
      if (win2.opener == null) childWindow.opener = self;
  }
  ...
  function CetakDaftar(id) {
    var _rnd = randomString();
    lnk = "$_SESSION[mnux].daftar.php?id="+id+"&_rnd="+_rnd;
    win2 = window.open(lnk, "", "width=700, height=500, scrollbars, status");
    if (win2.opener == null) childWindow.opener = self;
  }
