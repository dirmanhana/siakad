...
function GabungkanScript($md, $id, $resid, $w) {
  echo <<<ESD
  <script>
  function Gabung() {
    window.location="../$_SESSION[mnux].editlab.php?gos=Simpan&md=$md&id=$id&resid=$resid"+
      "&KuliahTanggal=$w[KuliahTanggal]&JenisJadwalID=$w[JenisJadwalID]"+
      "&HariID=$w[HariID]&JamMulai=$w[JamMulai]&JamSelesai=$w[JamSelesai]&RuangID=$w[RuangID]&Kapasitas=$w[Kapasitas]&DosenID=$w[DosenID]"+
      "&_RencanaKehadiranRes=$w[_RencanaKehadiranRes]&_MaxAbsenRes=$w[_MaxAbsenRes]&KuliahTanggal=$w[KuliahTanggal]"+
      "&_Gabungkan=721222";
  }
  function Kembali() {
    window.location="../$_SESSION[mnux].editlab.php?gos=Edit&md=$md&id=$id&resid=$resid&Kembali=1"+
...
function Simpan($md, $id, $resid, $Kembali) {
  ...
  if ($_Gabungkan == 721222) {
    //$w['TglMulai'] = sqling($_REQUEST['TglMulai']);
		...
		$w['KuliahTanggal'] = sqling($_REQUEST['KuliahTanggal']); //"$_REQUEST[KuliahTanggal_y]-$_REQUEST[KuliahTanggal_m]-$_REQUEST[KuliahTanggal_d]";
		$w['JamMulai'] = "$_REQUEST[JamMulai_h]:$_REQUEST[JamMulai_n]";
		$w['JamSelesai'] = "$_REQUEST[JamSelesai_h]:$_REQUEST[JamSelesai_n]";
