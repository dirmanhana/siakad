...
function Simpan(){
	...
	if ($_REQUEST[md] == 0){
		$s = "update tabimbingan set TglBimbingan = '".$TglBimbingan."', Catatan = '".$_REQUEST[Catatan]."'
				where BimbinganID = '".$_REQUEST[id]."' and TAID = '".$_REQUEST[TAID]."'";
		$q = _query($s);
	}
	if ($_REQUEST[md] == 1){
		$s = "insert into tabimbingan (TAID,TglBimbingan,Catatan,LoginBuat,TanggalBuat)
				values('".$_REQUEST[TAID]."','".$TglBimbingan."','".$_REQUEST[Catatan]."','".$_SESSION[_Login]."',now())";
		$q = _query($s);
	}
  echo <<<ESD
  <script>
