...
function TampilkanHeaderPresensi() {
    ...
    $optprodi = ($_SESSION['_LevelID'] == 100) ? GetOption3("prodi", "ProdiID", "concat(ProdiID, ' - ', Nama) as NM", "NM", $_SESSION['ProdiID'], '.') : GetProdiUser($_SESSION['_Login'], $_SESSION['ProdiID']);
...
function DftrJadwal() {
    ...
    if (empty($_SESSION['TahunID']) || empty($_SESSION['ProdiID']))
        echo Konfirmasi("Tahun Akademik & Program Studi", "Masukkan Tahun Akademik & Program Studi terlebih dahulu untuk dapat menampilkan jadwal kuliah.");
    else
        DftrJadwal1();
}
...
function DftrJadwal1() {
    ...
    $whr_prodi = ($_SESSION['ProdiID'] == "ALL") ? " " : " and j.ProdiID = '$_SESSION[ProdiID]' ";
    ...
    while ($w = _fetch_array($r)) {
      ...
      <td $c>$w[ProgramID]<sub><abbr>$w[ProdiID]</abbr></sub></td>
      <td $c>$w[DSN]</td>
...
function Edit() {
    ...
    left outer join jenisjadwal jj on jj.JenisJadwalID = j.JenisJadwalID 
	LEFT OUTER JOIN kelas k ON k.KelasID = j.NamaKelas
	", "j.JadwalID", $JadwalID, "j.*, concat(d.Nama, ' <sup>', d.Gelar, '</sup>') as DSN,
    ...
    if (empty($jdwl))
        die(ErrorMsg('Error', "Jadwal tidak ditemukan.<br />
    ...
    if ($jdwl['Final'] == 'Y')
        die(ErrorMsg('Error', "Jadwal sudah difinalisasi.<br />
...
function GetOption3($table, $key, $Fields, $Label, $Nilai, $Separator = ',', $whr = '', $antar = '<br />') {
    ...
    $r = _query($s);
    $str = '<option value="ALL"> Semua Prodi </option>';
    while ($w = _fetch_array($r)) {
        $_ck = ($w[$key] == $Nilai) ? 'selected' : '';
