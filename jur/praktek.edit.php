...
function GetDateOption3($value,$name){
  ...
  <tr><td class=inp>Dosen Pembimbing:</td>
      ...
      <input type=text name='Dosen' value='$w[Dosen]' size=30 maxlength=50 onKeyUp="javascript:CariDosen('$_SESSION[ProdiID]', 'frmPraktek')" />
      &raquo;
      <a href='#'
        onClick="javascript:CariDosen('$_SESSION[ProdiID]', 'frmPraktek')" />Cari...</a> |
  ...
  elseif ($md == 1) {
    ...
    if (empty($ada)) {
      $s = "insert into praktekkerja
		...
		NamaPekerjaan, Deskripsi, Pembimbing,
        LoginBuat, TanggalBuat)
        values
        ('$TahunID', '".KodeID."', '$MhswID',
        '$TglDaftar', '$TglMulai', '$TglSelesai',
        '$NamaPerusahaan', '$AlamatPerusahaan', '$KotaPerusahaan', '$TeleponPerusahaan', 
		'$NamaPekerjaan', '$Deskripsi', '$DosenID',
        '$_SESSION[_Login]', now())";
      $r = _query($s);
      TutupScript();
    }
