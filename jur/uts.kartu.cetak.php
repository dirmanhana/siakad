...
include_once "../header_pdf_indra.php";
...
function BuatHeaderDetail($p) {
    ...
    $p->Cell(8, $t, 'SKS', $l, 0);
    $p->Cell(70, $t, 'Dosen Pengampu', $l, 0);
    //$p->Cell(15, $t, 'H.UTS', $l, 0);
    //$p->Cell(15, $t, 'Tgl.UTS', $l, 0);
    $p->Cell(30, $t, 'TTD Pengawas', $l, 0, 'C');
...
function BuatIsinya($khs, $p) {
    ...
    where k.KHSID = $khs[KHSID] and k.Setara != 'Y'
...
function BuatFooter($khs, $jml, $sks, $p) {
    global $arrID;
    $MaxSKS = GetaField('maxsks', "KodeID='" . KodeID . "' and NA = 'N' and DariIP <= $khs[IPS] and $khs[IPS] <= SampaiIP and ProdiID", $khs['ProdiID'], 'SKS') + 0;
    // Pejabat
    $pjbt = GetFields('pejabat', "KodeID='" . KodeID . "' and KodeJabatan", 'KAPRODI_' . $khs['ProdiID'], "*");
    // Array Isi
    $tgl = date('d M Y');
    $arr = array();
    $arr[] = array('', '', '', $arrID['Kota'] . ', ' . $tgl);
    $arr[] = array('', '', '', $pjbt['Jabatan']); // A.n. KETUA
//    $arr[] = array('','','','~IMG~');
    $arr[] = array('Penting : ', '', '', '');
    $arr[] = array('1. Kartu ini wajib dibawa pada saat ujian', '', '', '');
    $arr[] = array('2. Ketinggalan kartu wajib lapor ke BAP-PMP', '', '', $pjbt['Nama']);
    $arr[] = array('3. Kehilangan kartu dikenakan denda Rp. 100.000', '', '', 'NIP: ' . $pjbt['NIP']);
