...
function Edit($md, $id, $Kembali) {
  if($Kembali == 1) {	  
	  ...
	  if ($md == 0) {
			...
			$jdl = "Tambah Jadwal";
			$thn = GetFields('tahun', "KodeID='".KodeID."' and NA='N' and ProgramID='$_SESSION[_jdwlProg]' and ProdiID", $_SESSION['_jdwlProdi'], 'TglKuliahMulai, TglKuliahSelesai');			
			$w = array();
			$w['Dosen'] = '';
			...
			$w['TglMulai'] = $thn['TglKuliahMulai']; //date('Y-m-d');
			$w['TglSelesai'] = $thn['TglKuliahSelesai']; //date('Y-m-d');
			$w['HariID'] = date('w');
			$w['AdaResponsi'] = 'N';
			$w['BiayaKhusus'] = 'N';
	  }
...
function GabungkanScript($md, $id, $w) {
  ...
  function Gabung() {
    window.location="../$_SESSION[mnux].edit.php?gos=Simpan&md=$md&id=$id&TahunID=$w[TahunID]"+
      ...
      "&BiayaKhusus=$w[BiayaKhusus]&Biaya=$w[Biaya]&NamaBiaya=$w[NamaBiaya]&TglMulai=$w[TglMulai]&TglSelesai=$w[TglSelesai]&AdaResponsi=$w[AdaResponsi]"+
      "&_Gabungkan=721222";
  }
  function Kembali() {
