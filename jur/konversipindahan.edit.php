...
function Edit($md, $mhsw, $thn, $id) {
  ...
  $optnilai = GetOption2('nilai', "concat(Nama, ' ', char(133), ' ', Bobot)", "Bobot desc", $w['BobotNilai'], "ProdiID='$m[ProdiID]' and KodeID='".KodeID."'", 'Bobot');
  // Tampilkan
  ...
  <tr><td class=inp width=120>Tahun Akd:</td>
      <td class=ul><input type=text name='thn' value='$w[TahunID]' size=6 maxlength=6 /></td>
			<!--<input type=button name='savethn' value=''>-->
      </tr>
  <tr><td class=inp>Matakuliah:</td>
      ...
      <input type=text name='SKS' value='$w[SKS]' size=3 maxlength=3> <sub>SKS</sub>
        <div style='text-align:right'>&raquo;<a href='#' onClick="javascript:CariMK('$m[ProdiID]', 'frmEdit')" />Cari...</a> |
...
function UrutSesi($mhsw, $m, $thn) {
  $Sesi = 1;
  $s = "select * from khs where MhswID = '$mhsw' order by TahunID";
  $r = _query($s);
  while ($w = _fetch_array($r)) {    
    $sx = "update khs set Sesi = '$Sesi' where MhswID = '$mhsw' and TahunID = '$w[TahunID]'";
    $rx = _query($sx);    
    $Sesi++;
  } 
}
