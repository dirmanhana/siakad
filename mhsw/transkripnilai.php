...
function CekPembayaran($nim) {
    ...
    $jumlahParam = $d_query['Jumlah'];
    $besarParam = $d_query['Besar'];
    $dibayarParam = $d_query['Dibayar'];
    $potongan = $d_query2['potongan'];
    
    $jumlah_besar1 = $jumlahParam*$besarParam;
    $dibayar_potongan1 = $dibayarParam+$potongan;

    //Cek Pembayaran SKS
    $cek_query = "SELECT TagihanID, Jumlah,Besar, Dibayar  FROM bipotmhsw WHERE 
            TahunID='" . $_SESSION['_krsTahunID'] . "' and 
            MhswID ='" . $_SESSION['_krsMhswID'] . "' and 
            BIPOTNamaID='3' and NA='N'";
    $r_query = _query($cek_query);
    $d_query = _fetch_array($r_query);
    ...
    $jumlah_besar2 = $jumlahParam*$besarParam;
    $dibayar_potongan2 = $dibayarParam+$potongan;

    if ((($jumlah_besar1+$jumlah_besar2) > ($dibayar_potongan1 + $dibayar_potongan2))) {
        echo ErrorMsg('Perhatian', "Maaf pembayaran belum lunas, untuk sementara tidak dapat melihat nilai anda.<br />
        Hubungi Sysadmin untuk informasi lebih lanjut.");
        return false;
    }
    else
...
function TampilkanDaftarKRSMhsw($thn, $mhsw, $khs) {
    $s = "select k.KRSID, k.MKKode, k.Nama, k.NilaiAkhir, k.BobotNilai, k.GradeNilai, k.SKS, k.Tinggi
    ...
    where k.KodeID = '" . SISFO . "'
      and k.MhswID = '" . $_SESSION[_Login] . "'
      and k.Tinggi = '*'
      and k.TahunID <= '".$thn["TahunID"]."'
    order by k.MKKode";
    $r = _query($s);
    $n = 0;
...
ESD;
        ...
        $jmlsks = $jmlsks + $w[SKS];
        $jmlbobot = $jmlbobot + ($w[SKS] * $w[BobotNilai]);
    }

    $ips = round(($jmlbobot / $jmlsks), 2);

    echo <<<ESD
    <tr>
        <td class=inp width=30>&nbsp;</td>
        <td class=ul width=100>&nbsp;</td>
        <td class=ul><b>Jumlah</b></td>
        <td class=ul align=right width=20><b>$jmlsks</b></td>
        <td class=ul align=right width=30>&nbsp;</td>
        <td class=ul width=30 align=center>&nbsp;</td>
        <td class=ul width=30 align=right><b>$jmlbobot</b></td>
        </tr>
ESD;
  ...
  <tr>    
      <td class=inp width=80><b>Jumlah SKS : </b></td>
      <td class=ul width=200><b>$jmlsks</b></td      
  </tr>
  <tr>      
      <td class=inp width=80><b>Jumlah Bobot : </b></td>
      <td class=ul width=200><b>$jmlbobot</b></td 
  <tr>
  <tr>      
      <td class=inp width=80><b>Indeks Prestasi Komulatif (IPK) : </b></td>
      <td class=ul width=200><b>$ips</b></td 
  <tr>          
  </table>
ESD;
}
