...
function GetNextNIM($TahunNIM, $mhsw) {
  ...
  // Ubah kode Status Awal2 Masuk Mhsw (2 = Mahasiswa Transfer; 1 = Non Transfer)
  $StatusAwalID2 = ($mhsw['StatusAwalID2'] == 'Y')? '7' : '1';
  $tmp = str_replace('~T~', $StatusAwalID2, $tmp);    
    
  // untuk check
  $check = $tmp; // '2012201____'; 20122012001
  $check = str_replace('~NMR3~', '___', $check);
  $check = str_replace('~NMR4~', '____', $check);
  $check = str_replace('~NMR5~', '_____', $check);
  $check2 = substr(trim($check), 0, strlen(trim($check))-3).'___';    
  $xxx2 = $check2;
  
  // check dulu
  $s = "select max(concat(left(trim(MhswID),length(trim(MhswID))-4),right(trim(MhswID),3))) as LAST from mhsw where MhswID like '$check2' ";
  $r = _query($s);
  $w = _fetch_array($r);        
  $xxx = $w['LAST'];
  if (empty($w['LAST'])) {        
    ...
    $xx = substr(trim($_lst), strlen(trim($_lst))-3, 3);       
    $nextincr = $xx + 1;
    $nextincr2 = str_pad($nextincr, 3, '0', STR_PAD_LEFT);    
    $_lst = str_replace($base, '', $_lst) +1;    
    // Format jumlah digit
    $Last = $tmp; // 20122011~NMR3~
    ...
    $Last2 = substr(trim($Last), 0, strlen(trim($Last))-3);    
    $Last3 = $Last2 . $nextincr2;
    $Last = $Last3;
  }
  return $Last;//.'.'.$xxx;
}
...
function GetNextNIMSementara($TahunNIM, $mhsw) { 
  ...
  // untuk check  
  $check = $tmp; // // 'eg. 12TEMPTGF2~NMR3~'
  $check = str_replace('~NMR3~', '___', $check);
  $check = str_replace('~NMR4~', '____', $check);
  $check = str_replace('~NMR5~', '_____', $check);  
  $check2 = substr(trim($check), 0, strlen(trim($check))-3).'___'; // 'eg. 12TEMPTGF____' dan '1208S____'; 
    
  // check dulu  
  $s = "select max(concat(left(trim(MhswID),length(trim(MhswID))-4), right(trim(MhswID),3))) as LAST from mhsw where MhswID like '$check2' "; // select max(concat('1208S', '[NMR3]')) like '1208S____'
  $r = _query($s);
  $w = _fetch_array($r);  
  if (empty($w['LAST'])) {
    $Last = $tmp;
    ...
    $_lst = $w['LAST']; // '1208S2001'    
    $base = $tmp; // '1208S2~NMR3~'
    $base = str_replace('~NMR3~', '', $base); // '1208S2'
    $base = str_replace('~NMR4~', '', $base);
    $base = str_replace('~NMR5~', '', $base);
    $xx = substr(trim($_lst), strlen(trim($_lst))-3, 3); // '001'    
    $nextincr = $xx + 1; // 2    
    $nextincr2 = str_pad($nextincr, 3, '0', STR_PAD_LEFT); // '002'
    //$xxx = 'base='.$base.' - '.'_lst='.$_lst; // base=201308S1, _lst=201308S001 seharusnya base=201308S
    $base = substr($base, 0, strlen(trim($base))-1);
    $_lst = str_replace($base, '', $_lst) +1; // '2001'+1 = '2002', tanpa +1 = 201308S001     
    //$_lst = str_replace('201308S', '', $_lst) ; // 001
    //$xxx = $_lst;
    
    // Format jumlah digit
    $Last = $tmp; // '1208S2~NMR3~'
    $Last = str_replace('~NMR3~', str_pad($_lst, 3, '0', STR_PAD_LEFT), $Last); // '1208S2002'
    $Last = str_replace('~NMR4~', str_pad($_lst, 4, '0', STR_PAD_LEFT), $Last);
    $Last = str_replace('~NMR5~', str_pad($_lst, 5, '0', STR_PAD_LEFT), $Last);
    $Last2 = substr(trim($Last), 0, strlen(trim($Last))-3); // '1208S2'
    $Last3 = $Last2 . $nextincr2; // '1208S2' . '002'
    $Last = $Last3; // '1208S2002'
  }  
  return $Last;
}
...
function GetNextTagihan() {
  global $_BPMDigit;
  $_BPMDigit = (empty($_BPMDigit))? 5 : $_BPMDigit;
  $thn = date('Y').'-';
  $panjang = str_pad('_', $_BPMDigit, '_');
  $bpmmx = GetaField('cetaktagihan', "NoCetakTagihan like '".$thn.$panjang."' and KodeID", 
    KodeID, "max(NoCetakTagihan)");
  $bpmcnt = str_replace($thn, '', $bpmmx)+1;
  $bpmcnt = $thn.str_pad($bpmcnt, $_BPMDigit, '0', STR_PAD_LEFT);
  return $bpmcnt;
}
...
function GetOption333($_table, $_field, $_order='', $_default='', $_where='', $_value='', $not=0, $blank=1) {
  global $strCantQuery;
	if (!empty($_order)) $str_order = " order by $_order ";
	else $str_order = "";
	if (!empty($_where)) {
