...
function GetDateOption($dt, $nm='dt') {
  ...
  return "<select name='".$nm."_d' id='".$nm."_d'>$_dy</select>
    <select name='".$nm."_m' id='".$nm."_m'>$_mo</select>
    <select name='".$nm."_y' id='".$nm."_y'>$_yr</select>";
}
...
function GetArrayTable($sql, $key, $label, $separator=', ', $diapit='', $label2='') {
  ...
  while ($w = _fetch_array($r)) {
    $ret .= $diapit . $w[$label] .$ap1 . $w[$label2] . $ap2 . $diapit. $separator;
  }
...
function Konfirmasi1($isi) {
  ...
  <tr><th class=ttl><span>$isi</span></th></tr>
  </table></p>";
}
function Konfirmasi($jdl, $isi) {
  ...
  //return "<div class=mod><h1><span>$jdl</span></h1><div class=form><div style='float:left;padding:5px;min-height:100px'><img src='img/tux001.jpg' /s></div><div style='float:left'>$isi</div><div class=clear></div></div></div>";
}
...
function ErrorMsg($jdl='Error', $isi='') {
  Return "<p><table class=box cellspacing=1 cellpadding=4 width=100%>
  <tr><td rowspan=2 width=10><img src='img/warn.png'></td>
  <td bgcolor=#FF3F00><font color=yellow><strong>$jdl</strong></font></td>
  <tr><td>$isi</td></tr>
  </table></p>";
}
...
function GetNextPMBID($prodi='') {
  ...
  $pmbmx = GetaField('pmb', "PMBID like '$pmbaktif%' and NA", 'N', "max(PMBID)");
...
function GetNextNIM($TahunNIM, $mhsw) {
  ...
  $thn = substr($TahunNIM, 0, 4);
  $tmp = $stp;
  $tmp = str_replace('~YY~', substr($TahunNIM,3,2), $tmp);
  $tmp = str_replace('~YYYY~', $thn, $tmp);
  // Ubah kode program
...
function TampilkanPilihanProdi($mnux='', $gos='', $pref='', $token='') {
  ...
  //$opt = GetOption2("prodi", "concat(ProdiID, ' - ', Nama)", "ProdiID", $_SESSION['prodi'], "KodeID='$arrID[Kode]' and ProdiID in ($_prodi)", 'ProdiID');
   $opt = GetOption2("prodi", "concat(ProdiID, ' - ', Nama)", "ProdiID", $_SESSION['prodi'], "KodeID='$arrID[Kode]'", 'ProdiID');
...
function TampilkanPilihanProdiAngkatan($mnux) {
  global $arrID;
...
function NamaTahun($tahun, $prodi='') {
  $arr = array('1'=>'Ganjil', '2'=>'Genap', '3'=>'Pendek Ganjil', '4'=>'Pendek Genap');
