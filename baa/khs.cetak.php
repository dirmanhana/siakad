...
function BuatFooter($khs, $p) {
  ...
  $SKSPerolehan = GetaField("krs k left outer join khs h on k.KHSID=h.KHSID and h.KodeID='".KodeID."'", "k.MhswID='$khs[MhswID]' and k.Tinggi='*' and (h.Sesi <= $khs[Sesi] or k.KHSID=0) and k.KodeID",
    KodeID, "sum(k.SKS)");
  $SKSLulus = GetaField("krs k left outer join khs h on k.KHSID=h.KHSID and h.KodeID='".KodeID."'", "k.MhswID='$khs[MhswID]' and k.Tinggi='*' and (h.Sesi <= $khs[Sesi] or k.KHSID=0) and k.GradeNilai != 'E' and k.KodeID",
    KodeID, "sum(k.SKS)");	
  ...
  $pjbt = GetFields('pejabat', "KodeID='".KodeID."' and NA='N' and KodeJabatan", 'KAPRODI_'.$khs['ProdiID'], "*");
  // Array Isi
  $tgl = date('d M Y');
  $arr = array();
