...
function DftrBipotNama() {
  ...
  $ka = AmbilBipot(-1);
  // Tampilkan
  echo "<p><a href='?sub=BipotNamaEdt&md=1&mnux=$mnux&tok=$tok'>Tambahkan Nama Biaya/Potongan</a>";
  echo "<table class=bsc cellspacing=1 cellpadding=4 width=100%>
...
function AmbilBipot($TrxID=1) {
  ...
  while ($w = _fetch_array($r)) {
    ...
    $a .= "<tr><td class=inp width=20>$w[Urutan]</td>
      <td $c width=10><a href='?mnux=$mnux&tok=$tok&sub=BipotNamaEdt&md=0&bnid=$w[BIPOTNamaID]'><img src='img/edit.png'></a></td>
      <td $c>$w[Nama]</td>
...
function BipotNamaEdt() {
  ...
  <tr><td class=inp>Di baris</td><td class=ul><input type=text name='Baris' value='$w[Baris]' size=3 maxlength=3> Pada baris ke berapa dalam cetakan BPM Autodebet.</td></tr>
  <tr><td class=inp>Dipotong Beasiswa</td><td class=ul><input type=checkbox name='DipotongBeasiswa' value='Y' $DipotongBeasiswa> Apakah dapat dipotong beasiswa?</td></tr>
