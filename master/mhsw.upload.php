  ...
  {		
		echo "
		  ...
		  echo "<tr>
					...
					<th class=ttl width=50>ProgramID</th>
					<th class=ttl width=50>ProdiID</th>					
		  ...
		  for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {	
				$n++;
				$w = array(); 									
				$w['JenjangID'] = trim($data->sheets[0]['cells'][$i][2]);							
				$w['ProdiID'] = trim($data->sheets[0]['cells'][$i][3]);
				$w['MhswID'] = trim($data->sheets[0]['cells'][$i][4]);				
				if ($w['MhswID'] != '') {			
					$w['Nama'] = trim($data->sheets[0]['cells'][$i][5]);
					$w['ProgramID'] = trim($data->sheets[0]['cells'][$i][6]);
					$w['TempatLahir'] = trim($data->sheets[0]['cells'][$i][7]);
					$w['TanggalLahir'] = trim($data->sheets[0]['cells'][$i][8]);
					$w['Kelamin'] = trim($data->sheets[0]['cells'][$i][9]);					
					$w['TahunID'] = trim($data->sheets[0]['cells'][$i][10]);								
					$w['BatasStudi'] = trim($data->sheets[0]['cells'][$i][11]);
					$w['StatusMhswID'] = trim($data->sheets[0]['cells'][$i][12]);
					$w['StatusAwalID'] = trim($data->sheets[0]['cells'][$i][13]);							
					/*$w['TahunLulus'] = trim($data->sheets[0]['cells'][$i][10]);
					$w['PropinsiAsalID'] = trim($data->sheets[0]['cells'][$i][13]);
					$w['TanggalLulus'] = trim($data->sheets[0]['cells'][$i][15]);										
					$w['MhswIDAsalPT'] = trim($data->sheets[0]['cells'][$i][19]);		
					$w['AsalPT'] = trim($data->sheets[0]['cells'][$i][20]);		
					$w['ProdiAsalPT'] = trim($data->sheets[0]['cells'][$i][22]);*/		
					if ($w['MhswID'] == '') {
					...
					if ($w['ProdiID'] == '') {
						$TidakMemenuhiSyarat = true;
						$w['Keterangan'] = "ProdiID Kosong'";
					} else {
						$TidakMemenuhiSyarat = false;
						$w['Keterangan'] = '';
					}			
					// Cek apakah sudah dimigrasi?					
					$Data_exist = GetaField('mhsw', "MhswID='$w[MhswID]' and NA='N' and KodeID", KodeID, "MhswID"); 
					...
					if ($TampilkanBaris) { //TempatLahir, TanggalLahir, Kelamin, BatasStudi, TahunLulus, PropinsiAsalID, TanggalLulus, StatusMhswID, StatusAwalID
						echo "<tr>
								...
								<td class=$class align=center>$w[ProgramID]<input type=hidden name='ProgramID$n' value='$w[ProgramID]'></td>		
								<td class=$class align=center>$w[ProdiID]<input type=hidden name='ProdiID$n' value='$w[ProdiID]'>
									<input type=hidden name='JenjangID$n' value='$w[JenjangID]'>
									<input type=hidden name='TempatLahir$n' value='$w[TempatLahir]'> 
									<input type=hidden name='TanggalLahir$n' value='$w[TanggalLahir]'>
									<input type=hidden name='Kelamin$n' value='$w[Kelamin]'> 
									<input type=hidden name='BatasStudi$n' value='$w[BatasStudi]'>
									<input type=hidden name='StatusMhswID$n' value='$w[StatusMhswID]'> 
									<input type=hidden name='StatusAwalID$n' value='$w[StatusAwalID]'> 
									<!--
									<input type=hidden name='TahunLulus$n' value='$w[TahunLulus]'> 
									<input type=hidden name='PropinsiAsalID$n' value='$w[PropinsiAsalID]'>
									<input type=hidden name='TanggalLulus$n' value='$w[TanggalLulus]'> 									
									<input type=hidden name='MhswIDAsalPT$n' value='$w[MhswIDAsalPT]'>
									<input type=hidden name='AsalPT$n' value='$w[AsalPT]'> 
									<input type=hidden name='ProdiAsalPT$n' value='$w[ProdiAsalPT]'>
									-->
								</td>			
...
function Simpan() { 
  ...
  for($i = 1; $i <= $JumlahData; $i++) {	
		...
		if(!empty($CheckBox)) {	
			$JenjangID = $_REQUEST['JenjangID'.$i];
			$MhswID = '00'.$_REQUEST['MhswID'.$i];
			$TahunID = $_REQUEST['TahunID'.$i];
			$Nama = $_REQUEST['Nama'.$i];						
			$ProgramID = $_REQUEST['ProgramID'.$i];						
			$ProdiID = $_REQUEST['ProdiID'.$i];				
			$TempatLahir = $_REQUEST['TempatLahir'.$i];				
			$TanggalLahir = $_REQUEST['TanggalLahir'.$i];				
			$Kelamin = $_REQUEST['Kelamin'.$i];
			$BatasStudi = $_REQUEST['BatasStudi'.$i];
			$StatusMhswID = $_REQUEST['StatusMhswID'.$i];
			$StatusAwalID = $_REQUEST['StatusAwalID'.$i];
			/*$TahunLulus = $_REQUEST['TahunLulus'.$i];
			$PropinsiAsalID = $_REQUEST['PropinsiAsalID'.$i];
			$TanggalLulus = $_REQUEST['TanggalLulus'.$i];			
			$MhswIDAsalPT = $_REQUEST['MhswIDAsalPT'.$i];
			$AsalPT = $_REQUEST['AsalPT'.$i];
			$ProdiAsalPT = $_REQUEST['ProdiAsalPT'.$i];*/
			...
			if ($Sync == 1) { // untuk proses sinkronisasi				
				...
				// ------------				
				/*$s = "insert into mhsw
							(MhswID, TahunID, Nama, ProgramID, ProdiID, TempatLahir, TanggalLahir, Kelamin, BatasStudi, TahunLulus, PropinsiAsalID, TanggalLulus, 
								StatusMhswID, StatusAwalID, MhswIDAsalPT, AsalPT, ProdiAsalPT, NIMSementara, LoginBuat, TanggalBuat, KodeID)
							values
							('$MhswID', '$TahunID', '$Nama', '$ProgramID', '$ProdiID', '$TempatLahir', '$TanggalLahir', '$Kelamin', '$BatasStudi', '$TahunLulus', '$PropinsiAsalID', '$TanggalLulus', 
								'$StatusMhswID', '$StatusAwalID', '$MhswIDAsalPT', '$AsalPT', '$ProdiAsalPT', 'N', '$_SESSION[_Login]', now(), '".KodeID."')";*/
				$s = "insert into mhsw
					(MhswID, TahunID, Nama, ProgramID, ProdiID, TempatLahir, TanggalLahir, Kelamin, BatasStudi, 
						StatusMhswID, StatusAwalID, LoginBuat, TanggalBuat, KodeID)
					values
					('$MhswID', '$TahunID', '$Nama', '$ProgramID', '$ProdiID', '$TempatLahir', '$TanggalLahir', '$Kelamin', '$BatasStudi', 
						'$StatusMhswID', '$StatusAwalID', '$_SESSION[_Login]', now(), '".KodeID."')";
				$r = _query($s);					
			} 										
