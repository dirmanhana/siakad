...
function PktSav() {
  ...
  $JumSksPaket = GetaField('mkpaket', "ProdiID = '$_SESSION[prodi]' and NA", 'N', "SUM(SKS)");
...
function IsiPkt() {
  ...
  /*
  $s = "
		select PresensiID, Topik from presensi where JadwalID = '$je[JadwalID]' and Topik != 'NULL' and Topik != 'tes' and Topik != '' and NA = 'N' and 
		NOT(PresensiID IN (select PresensiID from evaluasitopik where KodeID = '".KodeID."' and JadwalEvaluasiID = '$_GET[id]' and NA='N')) order by Pertemuan
		";
  */
  $s1 = "select Nama , MKKode, MKID, SKS from mk where NA='N' and KurikulumID='$arrPaket[KurikulumID]' and
		NOT(MKID IN (select MKID from mkpaketisi where KodeID = '".KodeID."' and MKPaketID = '$mpid' and NA='N'))
		order by Nama ";		
  $q1 = _query($s1);
  while ($w1 = _fetch_array($q1)){
  ...
  */	
  ...
  $jmlsks = GetFields("mkpaketisi mpi left outer join mk mk on mpi.MKID=mk.MKID", 
      'mpi.MKPaketID', 
      $_REQUEST[mpid], 
      "count(*) as JMLMK, sum(mk.SKS) as JMLSKS");
  $makssks = GetaField('prodi', "ProdiID = '$_SESSION[prodi]' and NA", 'N', "DefSKS");
...
function IsiPktAdd() {
  ...
  $jmlsks = $_REQUEST['jmlsks'];
  $makssks = $_REQUEST['makssks'];
  //$ada = GetFields('mkpaketisi', "KurikulumID='$kurid' and MKPaketID='$mpid' and MKID", $mkid, '*');
  $tambahjmlsks = GetaField('mk', "MKID='$mkid' and NA", 'N', 'SKS');
  ...
  if ($totalsks<=$makssks) {
   ...
   echo ErrorMsg("Tidak Disimpan", 
     ...
     <a href='?mnux=$_SESSION[mnux]&mk=Pkt&sub=IsiPkt&mpid=$mpid'>Kembali</a>");
