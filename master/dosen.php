...
function DsnEdtAkademik() {
  ...
  if ($_SESSION['_LevelID'] == 100) { // Dosen
    $disabled = "disabled";    
    $btnSimpan = "";
    $btnReset = "";    
    $TglBekerja = $w['TglBekerja'];
    $_ProdiID = $w['ProdiID'];
  } else {
    $disabled = "";    
    $btnSimpan = "<input type=submit name='Simpan' value='Simpan'>";
    $btnReset = "<input type=reset name='Reset' value='Reset'>";    
  ...
  <tr><td class=inp>Status Dosen</td>
      <td class=ul><select name='StatusDosenID' $disabled>$StatusDosen</select></td></tr>
  <tr><td class=inp>Status Kerja</td>
      <td class=ul><select name='StatusKerjaID' $disabled>$StatusKerja</select></td></tr>
  <tr><td class=inp>Prodi Homebase</td>
      <td class=ul><select name='Homebase' $disabled>$Homebase</select></td></tr>
  <tr><td class=inp>Mengajar di Prodi</td>
      <td class=ul>$_ProdiID</td></tr>
  <tr><td class=inp>Kode Instansi Induk</td>
      <td class=ul><input type=text name='InstitusiInduk' value='$w[InstitusiInduk]' size=10 maxlength=10 $disabled></td></tr>
  <tr><td class=inp>Lulus Perg. Tinggi</td>
      <td class=ul><input type=text name='LulusanPT' value='$w[LulusanPT]' size=40 maxlength=50 $disabled></td></tr>
  <tr><td class=inp>Jenjang Pendidikan Tertinggi</td>
      <td class=ul><select name='JenjangID' $disabled>$Jenjang</select></td></tr>
  <tr><td class=inp>Keilmuan</td>
      <td class=ul><input type=text name='Keilmuan' value='$w[Keilmuan]' size=40 maxlength=100 $disabled></td></tr>  
  
  <tr><td colspan=2 align=center>$btnSimpan
    $btnReset</td></tr>
  </form></table>";
}
...
function DsnEdtJabatan() {
  ...
  if ($_SESSION['_LevelID'] == 100) { // Dosen
    ...
    $btnTambahJabatanAkd = "";
    $btnTambahGolongan = "";
    $btnTambahTunjanganIkatan = "";
  } else {
    $disabled = "";    
    $btnSimpan = "<input type=submit name='Simpan' value='Simpan'>";
    $btnReset = "<input type=reset name='Reset' value='Reset'>";    
    $btnTambahJabatanAkd = "<a href='?mnux=$_SESSION[mnux]&gos=DsnEdt&md=1&dsnsub=JbtnEdt'>Tambah  </a>";
    $btnTambahGolongan = "<a href='?mnux=$_SESSION[mnux]&gos=DsnEdt&md=1&dsnsub=GolEdt'>Tambah  </a>";
    $btnTambahTunjanganIkatan = "<a href='?mnux=$_SESSION[mnux]&gos=DsnEdt&md=1&dsnsub=IktEdt'>Tambah  </a>";
  }
  echo "<p><table class=box cellspacing=1 cellpadding=4 width=600>
  <form action='?' method=POST>
  <input type=hidden name='mnux' value='$_SESSION[mnux]'>
  ...
  <tr><td class=inp>Jabatan Akademik</td>
      <td class=ul><select name='JabatanID' $disabled>$Jabatan</select> $btnTambahJabatanAkd</td></tr>
  <tr><td class=inp>Jabatan Dikti</td>
      <td class=ul><select name='JabatanDiktiID' $disabled>$JabatanDikti</select></td></tr>
  <tr><td class=inp>Golongan</td>
      <td class=ul><select name='GolonganID' $disabled>$optgol</select> $btnTambahGolongan</td></tr>
  <tr><td class=inp>Tunjangan Ikatan</td>
      <td class=ul><select name='IkatanID' $disabled>$optikt</select> $btnTambahTunjanganIkatan</td></tr>
  
  <tr><td class=ul colspan=2><b>Bank</b></td></tr>
  <tr><td class=inp>Nama Bank</td>
      <td class=ul><input type=text name='NamaBank' value='$w[NamaBank]' size=50 maxlength=50 $disabled></td></tr>
  <tr><td class=inp>Nama Akun</td>
      <td class=ul><input type=text name='NamaAkun' value='$w[NamaAkun]' size=50 maxlength=50 $disabled></td></tr>
  <tr><td class=inp>Nomer Akun</td>
      <td class=ul><input type=text name='NomerAkun' value='$w[NomerAkun]' size=50 maxlength=50 $disabled></td></tr>
...
function DsnEdtPendidikanSav() {
  ...
  if ($md == 0){
    $s = "update dosenpendidikan
      ...
      PerguruanTinggiID, NamaNegara, BidangIlmu, KodeBenua, Prodi, 
      LoginBuat, TanggalBuat)
  	  values ('$dsnid', $Nomor, '$Tglijazah', '$Gelar', '$JenjangID',
      '$PTID', '$NamaNegara', '$BidangIlmu', '$KodeBenua', '$ProdiDikti',
      '$_SESSION[_Login]', now())";
	  $r = _query($in); 
	}
  TampilPendidikan1();
...
function DsnEdtPenelitianSav() {
  ...
  if(empty($Penelitian)){
	die(ErrorMsg('Error',
		"Judul Penelitian Tidak boleh kosong.<br />
		Hubungi Sysadmin untuk informasi lebih lanjut.
		<hr size=1 color=silver />
		 <input type=button name='Tutup' value='Kembali' onClick=\"window.parent.location.href ='?mnux=$_SESSION[mnux]&gos=DsnEdt&dsnid=$dsnid&dsnsub=TampilTelitiDsn'\" />
		"));
  }else{
	  if ($md == 0){
...
function DsnEdtPekerjaan() {
  ...
  <tr><td class=inp1>Telepon</td><td class=ul><input type=text name='Telepon' value='$w[Telepon]' size=10 maxlength=15></td></tr>
  <tr><td class=inp1>Fax</td><td class=ul><input type=text name='Fax' value='$w[Fax]' size=10 maxlength=15></td></tr>
...
function DsnEdtPekerjaanSav() {
  ...
  if((empty($Institusi))or(empty($Jabatan))or(empty($Alamat))or(empty($Kota))){
    die(ErrorMsg('Error',
        "Nama Institusi, Jabatan, Alamat Institusi dan Kota Tidak boleh kosong.<br />
        Hubungi Sysadmin untuk informasi lebih lanjut.
        <hr size=1 color=silver />
         <input type=button name='Tutup' value='Kembali' onClick=\"window.parent.location.href ='?mnux=$_SESSION[mnux]&gos=DsnEdt&dsnid=$dsnid&dsnsub=TampilKerjaDsn'\" />
		"));
  }else{
	  if ($md == 0){
...
function DsnEdtPengajaran() {
  ...
  $lst->tables = "jadwal j
    ...
    where j.DosenID='$_SESSION[dsnid]' or (jd.DosenID = '$_SESSION[dsnid]')
    order by j.TahunID, j.HariID, j.JamMulai desc";
  //$NamaPT = GetaField('perguruantinggi','PerguruanTinggiID','=PerguruanTinggiID=','Nama');
  $lst->fields = "j.TahunID, j.MKKode, j.MKID, j.Nama, j.NamaKelas, j.JenisJadwalID, date_format(j.TglMulai, '$fmtTgl') as Tanggal, h.Nama as HR,
    j.JamMulai, j.JamSelesai,if(jd.DosenID = '08.0211.037','Team Teaching','Dosen Pengampu') as Status ";
  $lst->headerfmt = "<p><table class=box cellspacing=1 cellpadding=4>
    <tr><th class=ttl>#</th>
