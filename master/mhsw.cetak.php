...
function Isinya($prodi, $program, $srcmhswkey, $srcmhswval, $p) {
  ...
  {	  $n++;
	  ...
	  //$_Telepon = implode('/', $arrTelepon);
          ...
          if ((!empty($w['Handphone'])) && (strlen($w['Handphone']) <= 30)) {
              $arrTelepon[] = $w['Handphone'];
              if (strlen($w['Handphone']) > 15) {
                  $TanpaTelp = true;
              }
          }
          if ((!empty($w['Telepon'])) && (!$TanpaTelp)) {
              if (strlen($w['Telepon']) <= 15) {
                  $arrTelepon[] = $w['Telepon'];
              }
          }
	  ...
	  $p->Cell(10,  $t, $n, 1, 0, 'R');
	  $p->Cell(23, $t, $w['MhswID'], 1, 0, 'L');
	  $p->Cell(75, $t, strtoupper($w['Nama']), 1, 0, 'L');
	  $p->Cell(15, $t, $w['ProdiID'], 1, 0, 'C');
	  //$p->Cell(15, $t, $w['ProgramID'], 1, 0, 'C');
	  $p->Cell(12, $t, $w['StatusMhswID'], 1, 0, 'C', ($w['Keluar'] == 'N')?false:true);
          
          //AMR 20130818
          //ngilangin strip
          $printTelp = str_replace("-", "", $_Telepon);
          //ngilangin spasi
          $printTelp = str_replace(" ", "", $printTelp);
          //nambahin spasi / di telp
          $printTelp = str_replace("/", " / ", $printTelp);
	  $p->Cell(55, $t, $printTelp, 1, 0, 'L');
	  $p->Ln($t);
  }
}
...
{	$pjg = 110;
    ...
    $identitas = GetFields('identitas', 'Kode', KodeID, "Nama, CONCAT(Alamat1,' ',Alamat2,', ',Kota,' ',KodePos) AS Alamat, Telepon, Fax");
...
//    $p->Cell($pjg, 5, $identitas['Yayasan'], 0, 1, 'C');
	...
	$p->Cell($pjg, 3,
      $identitas['Alamat'], 0, 1, 'C');
//    $p->Cell($pjg, 3,
//      "Telp. ".$identitas['Telepon'].", Fax. ".$identitas['Fax'], 0, 1, 'C');
        $p->Cell($pjg, 3,
      "Telp. ".$identitas['Telepon'], 0, 1, 'C');
    $p->Ln(3);
