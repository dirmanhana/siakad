...
function DftrRuang() {
  ...
  */
  ...
  $s = "select r.RuangID, r.Nama, r.RuangKuliah, r.Kapasitas, r.KapasitasUjian,
...
function RuangSav() {
  ...
  if ($md == 0) {
    ...
    if (empty($ada)) {
      $s = "insert into ruang(KodeID, RuangID, Nama, ProdiID, KampusID, Lantai, RuangKuliah,
        Kapasitas, KapasitasUjian, KolomUjian, UntukUSM, Keterangan, NA)
        values('".KodeID."', '$RuangID', '$Nama', '$ProdiID', '$KampusID', '$Lantai', '$RuangKuliah',
        '$Kapasitas', '$KapasitasUjian', '$KolomUjian', '$UntukUSM', '$Keterangan', '$NA')";
      $r = _query($s);
      BerhasilSimpan("?mnux=$_SESSION[mnux]", 100);
    }
