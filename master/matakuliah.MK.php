...
function MKEdt() {
  ...
  <tr><td class=inp>Nama Matakuliah</td><td class=ul><input type=text name='Nama' value='$w[Nama]' style='text-transform: uppercase' size=40 maxlength=50></td></tr>
  <tr><td class=inp>Nama (Inggris)</td><td class=ul><input type=text name='Nama_en' value='$w[Nama_en]' size=40 maxlength=50></td></tr>
  ...
  <tr><td class=inp>Praktek Kerja?</td>
  ...
  <tr><td class=inp>Tugas Akhir?</td>
      ...
      *) Jika dicentang,<br />
      maka Mhsw harus mengambil MK ini utk dapat lulus
      </td></tr>
  ...
  if ($md == 0) {
    ...
    if (empty($ada)) {
      $s = "insert into mk (KurikulumID, KodeID, ProdiID, KonsentrasiID,
        ...
        PraktekKerja, Komprehensif, TugasAkhir, Keterangan,
...
function PraSav() {
	...
	if((!empty($PraID))or(!empty($NilaiID))or(!empty($MKAlternatif))){
	  if (!empty($PraID)) {
    ...
    die(ErrorMsg('Error',
        "Pilihan Tambah Prasyarat, Prasyarat Alternatif atau Nilai Minimal Tidak boleh kosong.<br />
