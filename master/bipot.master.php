...
function DftrBipot() {
  ...
  $a = "<table class=box cellspacing=1 cellpadding=4 width=100%>
  <tr><td colspan=6 class=ul1><a href='?mnux=$mnux&tok=$tok&sub1=BipotMasterEdt&md=1'>Tambah Master Biaya & Potongan</a></td></tr>
  <tr><th></th>
  ...
  while ($w = _fetch_array($r)) {
    ...
    $a .= "<tr>
      <td $c width=2>$_ki</td>
      <td $c align=center>
        <a href='?mnux=$mnux&tok=$tok&sub1=BipotMasterEdt&md=0&bipotid=$w[BIPOTID]'  title='Edit Master'><img src='img/edit.png'>
        <br />
      $w[Tahun]</a></td>
...
function HdrBipotIsi($JDL='', $TrxID) {
  ...
  return "<p><table class=box cellspacing=1 cellpadding=4 width=100%>
    ...
    <th class=ttl title='Otomatis'>
      <abbr title='Penarikan otomatis?'>Oto?</abbr>
      <hr size=1 color=silver />
      <abbr title='Menggunakan script?'>Scr?</abbr></th>
    <th class=ttl>Mulai</br>/ Brp<br />&times;</th>
    <th class=ttl>Stt<br />Awal
      <hr size=1 color=silver />
      Mhsw
      </th>
    <th class=ttl>Grade<br />USM</th>
	<th class=ttl>Grade<br />IPK</th>
    <th class=ttl>NA</th>
    </tr>";
}
  ...
  if (($arrbenar['ProgramID'] == $_SESSION['prid']) and ($arrbenar['ProdiID'] == $_SESSION['prodi'])) {
    ...
    while ($w = _fetch_array($r)) {
      ...
      <td $c nowrap align=center>
        <a href='?mnux=$mnux&tok=$tok&sub1=BipotIsiEdt&md=0&bipot2=$w[BIPOT2ID]&trxid=$w[TrxID]&trxnama=$w[NMTRX]'>
        $w[Prioritas]
        <br />
      ...
      <td $c align=right>
        $w[JML]
        <hr size=1 color=silver />
        <div align=left>$w[SAAT]</div>
        </td>
      <td $c align=center><img src='img/$w[Otomatis].gif'>
        <hr size=1 color=silver />
        $scr</td>
      <td $c align=right> $w[MulaiSesi] / $KaliSesi&times</td>
      <td $c>$w[StatusAwalID]
        <hr size=1 color=silver />
        $w[StatusMhswID]</td>
      <td $c align=center>
        <img src='img/$w[GunakanGradeNilai].gif'>
        <hr size=1 color=silver />
        $w[GradeNilai]&nbsp;</td>
	  <td $c align=center>
        <img src='img/$w[GunakanGradeIPK].gif'>
        <hr size=1 color=silver />
        $w[GradeIPK]&nbsp;</td>
...
function CopyProdiLainScript() {
  ...
  function CopyProdiLain(bipotid) {
    lnk = "$_SESSION[mnux].copyprodilain.php?bipotid="+bipotid;
...
function BipotIsiEdt() {
  ...
  $grdnilai = GetCheckboxes('pmbgrade', "GradeNilai", "concat(GradeNilai, ' ( ', Keterangan, ' ) ') as PMBGRD", "PMBGRD", $w['GradeNilai'], '.');
  $GunakanGradeIPK = ($w['GunakanGradeIPK'] == 'Y')? 'checked' : '';
  $grdipk = GetCheckboxes('gradeipk', "GradeIPK", "concat(GradeIPK, ' ( ', Keterangan, ' ) ') as IPKGRD", "IPKGRD", $w['GradeIPK'], '.');
  ...
  <tr><td class=inp>Prioritas Pembayaran</td><td class=ul><input type=text name='Prioritas' value='$w[Prioritas]' size=4 maxlength=3> Tampilan urutan/prioritas pada Tagihan</td></tr>
...
function BipotCopy() {
  ...
  <tr><td class=ul colspan=2>
    ...
    <input type=button name='Batal' value='Batal' onClick=\"location='?mnux=$mnux&tok=$tok'\">
    |
