...
if ($Sync == 1) {
	...
	TampilkanJudul("Migrasi Data - Master MK");
}
$gos($gel, $Sync);
...
function Simpan() { 
  ...
  for($i = 1; $i <= $JumlahData; $i++) {	
		...
		if(!empty($CheckBox)) {	
			$MKKode = str_replace(' ', $_REQUEST['MKKode'.$i]);//$_REQUEST['MKKode'.$i];
			...
			if ($Sync == 1) { // untuk proses sinkronisasi				
				...
				$s = "insert into mk
							(MKKode, Nama, Sesi, SKS, ProdiID, SKSTatapMuka, SKSPraktikum, KurikulumID, KonsentrasiID, Wajib, Responsi, JenisMKID, 
								JenisPilihanID, JenisKurikulumID, LoginBuat, TglBuat, KodeID)
							values
							('$MKKode', '$Nama', '$Sesi', '$SKS', '$ProdiID', '$SKSTatapMuka', '$SKSPraktikum',	'$KurikulumID', '6', 'Y', 'N', '11', 
								'11', '7', '$_SESSION[_Login]', now(), '".KodeID."')";
