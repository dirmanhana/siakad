...
function DftrBipotIsi1() {
  global $mnux, $tok;
  $arrbenar = GetFields('beasiswa', 'BeasiswaID', $_SESSION['beasiswaid'], "ProgramID, ProdiID");
  if (($arrbenar['ProgramID'] == $_SESSION['prid']) and ($arrbenar['ProdiID'] == $_SESSION['prodi'])) {
    ...
    while ($w = _fetch_array($r)) {
      ...
      <td $c nowrap align=center>
        <a href='?mnux=$mnux&tok=$tok&sub1=BIPOTIsiEdt&md=0'>
        $cnt
        <br />
...
function BipotCopy() {
  ...
  $bipot = GetaField('beasiswa', 'BeasiswaID', $beasiswaid, "concat(Tahun, ' - ', Nama)");
  // Ambil Daftar
