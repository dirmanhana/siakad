...
function TampilkanFilterDosen($mnux='dosen', $add=1) {
  ...
  if ($_SESSION['_LevelID'] == 100) { // Dosen
    ...
    $stradd = $stradd;    
    $btn1 = "<input type=button name='btnExportXL' value='Export Daftar Dosen' onClick=\"location='$_SESSION[mnux].xl.php'\" />";
    $btn2 = "<input type=button name='btnRekeningDosen' value='Cetak Rek. Dosen' onClick=\"CetakRekeningDosen()\" />";
    $btn3 = "<input type=button name='btnRiwayatDosen' value='Cetak Profil Dosen' onClick=\"CetakProfilDosen()\" />	";
  }
...
function DaftarDosen($mnux='', $lnk='', $fields='') {
  ...
  //echo 'test='.$where;
  if ($_SESSION['_LevelID'] == 100) { // Dosen
    $where = " and Login = '$_SESSION[_Login]'";  
    $hom = "";
  } else {
    $where = $where;  
    $hom = $hom;
