...
function TampilkanFilterMhsw() {
    ...
    if ($_SESSION['_LevelID'] == 120) { // Mahasiswa
        ...
        $btnEksport = "<input type=button name='CetakEksport' value='Eksport' onClick=\"CetakEksportMhsw('$_SESSION[prodi]', '$_SESSION[prid]', '$_SESSION[srcmhswkey]', '$_SESSION[srcmhswval]')\">";
    }
  ...
  <script>
	...
	{	lnk = '$_SESSION[mnux].cetak.php?prodi='+prodi+'&program='+program+'&srcmhswkey='+srcmhswkey+'&srcmhswval='+srcmhswval;
        ...
        function CetakEksportMhsw(prodi, program, srcmhswkey, srcmhswval)
	{	lnk = '$_SESSION[mnux].eksport.php?prodi='+prodi+'&program='+program+'&srcmhswkey='+srcmhswkey+'&srcmhswval='+srcmhswval;
		  win2 = window.open(lnk, \"\", \"width=600, height=400, scrollbars, status, resizable\");
		  if (win2.opener == null) childWindow.opener = self;
	}
  </script>";
