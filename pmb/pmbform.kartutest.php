...
{  
  ...
  $ImagePath = $Foto; //GetaField('pmb', "MhswID='$pmb[MhswID]' and KodeID", KodeID, "Foto");
	if(empty($ImagePath))
	{	$p->SetXY($p->getX()+55,$p->getY()-27);
		$p->Cell(19,22,'Foto 2x3',1,1,'C');
		$p->Ln(4);
	} else {
		$p->Image('../'.$ImagePath, $p->getX()+55,$p->getY()-27, 19, 22);
		$p->Ln(1);
	}
