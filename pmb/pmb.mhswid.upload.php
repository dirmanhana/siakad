...
HeaderSisfoKampus("Upload Data MhswID (dari format excel)");
...
// *** Main ***
if ($Sync == 1) {
	TampilkanJudul("Sinkronisasi Migrasi Data - MhswID 20121");
} else {
	TampilkanJudul("Migrasi Data - Master Ruang");
}
$gos($gel, $Sync);
  ...
  {		
		echo "
		  ...
		  for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {	
				...
				$w['MKKode'] = substr($w['MKKode'], 0, 1);
				$w['Nama'] = trim($data->sheets[0]['cells'][$i][4]);								
				if ($w['MKKode'] != '') {					
					$w['SKS'] = trim($data->sheets[0]['cells'][$i][5]);	
					$w['SKSTatapMuka'] = $w['SKS'];//trim($data->sheets[0]['cells'][$i][6]);
					$w['SKSPraktikum'] = 0;//trim($data->sheets[0]['cells'][$i][7]);	
...
function Simpan() { 
  ...
  for($i = 1; $i <= $JumlahData; $i++) {	
		...
		if(!empty($CheckBox)) {	
			...
			$KurikulumID = '';//GetaField('kurikulum', "KurikulumKode", $KurikulumKode, "KurikulumID"); 
			...
			if ($Sync == 1) { // untuk proses sinkronisasi		
				...
				while ($w = _fetch_array($r)) {							
					...
					$Tahun_Akd = '20121';
					//$NIM = (GetaField('prodi', 'ProdiID', $pmb['ProdiID'], 'GunakanNIMSementara')=='Y')?  GetNextNIMSementara($Tahun_Akd, $pmb) : GetNextNIM($Tahun_Akd, $pmb);
					$NIM = $w['MhswID'];//GetNextNIM($Tahun_Akd, $pmb);
					$sm = "update mhsw set MhswID='$NIM', Login='$NIM' where PMBID='$pmbid'";
					$rm = _query($sm);					
				}
				...
				if ($Data_exist) { // data sudah ada
					
				} else { // insert new mk
					$s = "insert into mk
								(MKKode, Nama, Sesi, SKS, ProdiID, SKSTatapMuka, SKSPraktikum, KurikulumID, LoginBuat, TglBuat, KodeID)
								values
								('$MKKode', '$Nama', '$Sesi', '$SKS', '$ProdiID', '$SKSTatapMuka', '$SKSPraktikum',	'$KurikulumID', 'MIGRASI', now(), '".KodeID."')";//$_SESSION[_Login]
					//$r = _query($s);	
				}
