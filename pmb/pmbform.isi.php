...
function Edit($md, $gel, $id) {
  ...
  if($_SESSION[_LevelID] == 50){
      $StatusAwalID = "PM";
      $where = " StatusAwalID = 'PM' ";
  }else{
      $StatusAwalID = $w['StatusAwalID'];
      $where = "";
  }
  $optstawal = GetOption2("statusawal", "concat(StatusAwalID, ' - ', Nama)", "StatusAwalID", $StatusAwalID, $where, "StatusAwalID");
  
  $statusawalid2checked = ($w['StatusAwalID2'] == 'Y')? 'checked' : '';
  ...
  <tr>
	  ...
	  <td class=ul1 colspan=3><input type=text id='aplikan' name='AplikanID' value='$w[AplikanID]'>
							  <input type=button name='CariAplikan' value='Cari Aplikan' onClick=\"carinomor(frmisi)\"/>
	  <br>
	  <font color=red>*) Dapat dicari melalui nomor aplikan atau nama aplikan.</font>
	  </td>
  </tr>
  <tr><td class=inp>Mahasiswa Transfer?</td>
	  <td class=ul1>
			<input type=checkbox name='StatusAwalID2' value='Y' $statusawalid2checked>
      </td>
  </tr>
  ...
  <tr><td class=inp>Nama Sekolah :</td>
	  <td class=ul1 colspan=3><input type=text name='NamaSekolah' value='$NamaSekolah' size=50 maxlength=255>
