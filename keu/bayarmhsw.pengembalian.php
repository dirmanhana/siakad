...
function Edit($MhswID, $KHSID, $md, $BayarID) {
  ...
  <form name='frmBayar' action='../$_SESSION[mnux].pengembalian.php' method=POST onSubmit=\"return CheckForm(this)\">
...
//    order by bm.TrxID, bm.BIPOTMhswID";
	...
	where bm.MhswID = '$MhswID'
      ...
      and bm.KodeID = '" . KodeID . "'
      and bm.NA = 'N'
      order by bm.TagihanID, bm.TrxID , bm.BIPOTMhswID"; //order by bm.TagihanID, b2.Prioritas, bm.TrxID DESC, bm.BIPOTMhswID
    $r = _query($s);
    $n = 0;
    $sisa = 0;
    $Dibayarx=0;
    ...
    while ($w = _fetch_array($r)) {
        if ($w['TrxID'] < 1) {
		  ...
		  <td $c align=right><a href='#' onClick=\"javascript:SetBayar($n, $selisih , $w[Dibayar]); HitungUlang();\">$SubTTL &raquo;</a></td>
    ...
    $_sisa = number_format(1 * $Dibayarx);   
    echo "<input type=hidden name='CNT' value='$n' />";
    echo "<input type=hidden name='sisane' value='$Dibayarx' />";   
    echo "<tr><td bgcolor=silver colspan=10 height=1></td></tr>";
    echo "<tr>
    <td class=ul1 colspan=4 align=right>Total yang Dapat Dikembalikan:</td>
    ...
    echo <<<ESD
  <script>
  function SetBayar(n, jml , dibayar) {
    eval("frmBayar.BYR_" + n + ".value = '" + (dibayar*1) +"';");
  }
  </script>
ESD;
    HitungUlang($n);
