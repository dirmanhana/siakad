...
$gos($MhswID, $TahunID, $md, $id, $tagihanid);

// *** Functions ***
function Edit($MhswID, $TahunID, $md, $id, $tagihanid) {
    if ($md == 0) {
        $jdl = "Edit Potongan Mhsw";
        $w = GetFields('bipotmhsw', 'BIPOTMhswID', $id, '*');
        $ro = "readonly=true disabled=true";
    } elseif ($md == 1) {
        $jdl = "Tambah Potongan Mhsw";
        $w = array();
        $w['Jumlah'] = 1;
        $ro = '';
    }
    ...
    $optbipotnama = GetOption2('bipotnama a inner join bipot2 b on a.BIPOTNamaID = b.BIPOTNamaID', "concat(a.Nama, ' (', a.TrxID, ')')", 'a.TrxID, a.Urutan', $w['BIPOTNamaID'], "a.KodeID='" . KodeID . "' and b.BIPOTID='$BIPOTID' and a.NA='N' and b.NA='N' and a.TrxID=-1", "concat(a.BIPOTNamaID,',',b.Jumlah)", 1); // concat(a.BIPOTNamaID,',',b.Jumlah)
  ...
  <form action='../$_SESSION[mnux].bipotedit2.php' method=POST>
  ...
  <tr><td class=inp>Potongan:</td><td class=ul1>
    $optbipotnama2
    $txtbipotnama
