...
function HitungUlangBIPOTMhsw($MhswID, $TahunID) {
    ...
    $biaya = GetaField("bipotmhsw bm
      left outer join bipot2 b2 on bm.BIPOT2ID = b2.BIPOT2ID", "bm.PMBMhswID = 1 and bm.KodeID = '" . KodeID . "'
      and bm.NA = 'N'
      and bm.TrxID = 1
      and bm.TahunID = '$TahunID' and bm.MhswID", $MhswID, "sum(bm.Jumlah * bm.Besar)") + 0;
    $potongan = GetaField("bipotmhsw bm
      left outer join bipot2 b2 on bm.BIPOT2ID = b2.BIPOT2ID", "bm.PMBMhswID = 1 and bm.KodeID = '" . KodeID . "'
      and bm.NA = 'N'
      and bm.TrxID = -1
      and bm.TahunID = '$TahunID' and bm.MhswID", $MhswID, "sum(bm.Jumlah * bm.Besar)") + 0;
    $bayar = GetaField('bayarmhsw', "PMBMhswID = 1 and KodeID = '" . KodeID . "'
      and NA = 'N'
      and TrxID = 1
      and TahunID = '$TahunID' and MhswID", $MhswID, "sum(Jumlah)") + 0;
    $tarik = GetaField('bayarmhsw', "PMBMhswID = 1 and KodeID = '" . KodeID . "'
      and NA = 'N'
      and TrxID = -1
      and TahunID = '$TahunID' and MhswID", $MhswID, "sum(Jumlah)") + 0;
    // Update data PMB
    ...
    $TotalBiaya = GetaField("bipotmhsw bm
      left outer join bipot2 b2 on bm.BIPOT2ID = b2.BIPOT2ID", "bm.KodeID = '" . KodeID . "'
      and bm.NA = 'N'
      and bm.TahunID = '$TahunID' and bm.MhswID", $MhswID, "sum(bm.TrxID * bm.Jumlah * bm.Besar)") + 0;

    $TotalBayar = GetaField('bayarmhsw', "KodeID = '" . KodeID . "'
      and NA = 'N'
      and TahunID = '$TahunID' and MhswID", $MhswID, "sum(Jumlah)") + 0;

    $TotalBayar = $TotalBayar - (-1 * $tarik);
