...
function DftrLapKeu() {
    $arrLap = array(
        ...
        'Laporan Tunggakan Mahasiswa~angsurmhsw',
        'Daftar Mahasiswa Yang Mendapatkan Beasiswa~beasiswa'
    );
    $i = 0;
    ...
    foreach ($arrLap as $arr) {
        ...
        $a = explode('~', $arr);
        $_b = "";
        if ($a[0] == "Laporan Tunggakan Mahasiswa") {
            $_b = "<a href='#$i' onClick=\"Excel('" . $a[1] . "', '" . $a[2] . "')\"><img src='img/excel.gif' /></a>";
        }
        $_a = "<a href='#$i' onClick=\"Prints('" . $a[1] . "', '" . $a[2] . "')\">";
        echo "<tr>
      <td class=inp width=10><a name='$i'></a>$i</td>
      <td class=ul1>$_a $a[0]</a></td>
      <td class=ul1 align=left width=50>
            $_a<img src='img/printer2.gif' /></a> &nbsp;&nbsp;
            $_b
      </td>
      </tr>";
    }
  ...
  function Excel(mdl, param) {
    var rnd = randomString();
    lnk = "$_SESSION[mnux]."+mdl+"Excel.php?TahunID=$_SESSION[TahunID]&ProdiID=$_SESSION[ProdiID]"+param+"&_rnd="+rnd;
    //window.location = lnk;
    win2 = window.open(lnk, "", "width=800, height=600, scrollbars, status");
    if (win2.opener == null) childWindow.opener = self;
  }
