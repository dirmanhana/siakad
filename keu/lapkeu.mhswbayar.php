...
function BuatIsinya($TahunID, $ProdiID, $p) {
  ...
  while ($w = _fetch_array($r)) {
    ...
    $p->Cell(23, $t, $w['MhswID'], 'B', 0);
    $p->Cell(46, $t, $w['NamaMhsw'], 'B', 0);
    $p->Cell(23, $t, $w['JML'], 'B', 0, 'R');
    $p->Cell(22, $t, $w['Bank'], 'B', 0);
    $p->Cell(44, $t, $w['BuktiSetoran'], 'BR', 0);
    $p->Ln($t);
  }
  $p->Cell($lbr, 1, ' ', 1, 1);
  $_ttl = number_format($ttl);
  $p->SetFont('Helvetica', 'B', 11);
  $p->Cell(93, 7, 'TOTAL :', 0, 0, 'R');
...
function BuatHeadernya($TahunID, $ProdiID, $p) {
  ...
  $p->Cell(23, $t, 'N I M', 1, 0);
  $p->Cell(46, $t, 'Nama Mahasiswa', 1, 0);
  $p->Cell(23, $t, 'Jumlah', 1, 0);
  $p->Cell(22, $t, 'Bank/Tunai', 1, 0);
  $p->Cell(44, $t, 'Nomer Bukti Setoran', 1, 0);
