...
function GetArrayTable($sql, $key, $label, $separator=', ') {
...
function ErrorMsg($jdl='Error', $isi='') {
  Return "<p>&nbsp;<br /><table class=box cellspacing=1 cellpadding=4 width=100%>
...
function GetNextBPM() {
  global $_BPMDigit;
  $_BPMDigit = (empty($_BPMDigit))? 4 : $_BPMDigit;
  $thn = date('Y');
...
function TampilkanTahunProdiProgram($mnux='', $gos='', $pref='', $token='') {
  global $arrID;
  if (empty($_SESSION['_ProdiID'])) $_prodi = '-1';
  else {
    $_ProdiID = trim($_SESSION['_ProdiID'], ',');
...
echo <<<EOF
  ...
  win2 = window.open("$namafile", "", "width=700, height=600, scrollbars, status");
  win2.creator = self;
  </SCRIPT>
EOF;
}
...
function HitungBatasStudi($TahunID, $prodi) {
  ...
  $_jmlthn = floor($prd['BatasStudi']/$prd['JumlahSesi']);
