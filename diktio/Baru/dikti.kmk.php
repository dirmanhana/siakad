  ...
  if ($jml > 0) {
    ...
    while ($w = _fetch_array($r)) {
      ...
      $dt = array(
        ...
        'Y',								// kode kurikulum
        'Y',								// Kode Kelompok Mata Kuliah
        $w['NIDN'],				

				$w['JenjangID'],
				$w['ProdiDiktiID'],
        
				'A',								// Status MK (A-aktif, H-hapus) 
        'Y',								// Ketersediaan Silabus
        'Y',								// Ketesedian satuan acara pengajaran (Y/T)
        'Y',								// Ketersediaan bahan ajar/Diktat (Y/T)
        ''									// Diktat
      );
