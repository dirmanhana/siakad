...
function HitungData() {
  ...
  $jml = GetaField("krs k
    ...
    "h.KodeID = '".KodeID."' $_prodi and k.NA", 'N', "count(KRSID)")+0;
  return $jml;
}
  ...
  if ($jml > 0) {
    ...
    while ($w = _fetch_array($r)) {
      ...
      $dt = array(
        ...
        $w['JenjangID'],
        $w['MhswID'],
        $w['NamaMhsw'], // *** Tambahan utk STKIP PGRI Pontianak
        $w['MKKode'],
        $w['NamaMK'],   // *** Tambahan utk STKIP PGRI Pontianak
        $w['DSN'],      // *** Tambahan utk STKIP PGRI Pontianak
        $w['GradeNilai'],
        $w['BobotNilai']
        );
