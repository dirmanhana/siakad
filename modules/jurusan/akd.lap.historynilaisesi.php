...
function TampilkanFilterNilai() {
  ...
  <input type=hidden name='mnux' value='akd.lap.historynilaisesi'>
...
function DaftarMhsw(){
  ...
  while ($w = _fetch_array($r)){
    ...
    $hdr = str_pad("*** Rekapitulasi Nilai ***", $maxcol, ' ', STR_PAD_BOTH) . $_lf.$_lf;
    ...
    fwrite($f, $div);
  	...
  	fwrite($f, str_pad("Akhir laporan", 100, ' ', STR_PAD_LEFT));
		fwrite($f, chr(12));
		$hal = 1;
...
function daftar(){
	...
	if (empty($Cekdl) && empty($tahun)) {
      ...
      "Tidak ada mahasiswa dengan NPM: <b>$_SESSION[mhswid]</b>");
		...
		$JmlSKS = GetFields('krs',"k.StatusKRSID='A' and GradeNilai<>'' and GradeNilai<>'-' and k.Final = 'Y' and MhswID",$_SESSION['mhswid'],"Sum(SKS) as jSKS ,SUM(SKS * BobotNilai) as jx ");
		//$JmlX = GetaField('krs')
		$hdr = str_pad("*** Rekapitulasi History Nilai Per Semester ***", $maxcol, ' ', STR_PAD_BOTH) . $_lf.$_lf;
