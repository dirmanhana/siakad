...
function DefJenKur() {
  ...
  if (!empty($_SESSION['prodi'])) {
    TampilkanMenuJenisKurikulum();
    TampilkanJenisKurikulum();
  }
}

function TampilkanMenuJenisKurikulum() {
  global $mnux, $pref, $token;
  echo "<p><a href='?mnux=$mnux&$pref=JenKur&sub=JenKurEdt&md=1'>Tambah Jenis Kurikulum</a> |
  <a href='?mnux=$mnux&$pref=JenKur&sub=CetakJenKur'>Cetak</a></p>";
}
function TampilkanJenisKurikulum() {
  ...
  while ($w = _fetch_array($r)) {
    ...
    echo "<tr><td class=ul><a href='?mnux=$mnux&$pref=JenKur&sub=JenKurEdt&md=0&JenisKurikulumID=$w[JenisKurikulumID]'><img src='img/edit.png'>
      $w[Singkatan]</a></td>
      <td $c>$w[Nama]</td>
      <td $c><img src='img/book$w[NA].gif'></td>
      </tr>";
  }
  echo "</table></p>";
}
function JenKurEdt() {
  ...
  <input type=hidden name='$pref' value='JenKur'>
  ...
  <tr><td class=ul colspan=2><input type=submit name='Simpan' value='Simpan'>
    ...
    <input type=button name='Batal' value='Batal' onClick=\"location='?mnux=$mnux&$pref=JenKur'\"></td></tr>
  </table></p>";
}
function JenKurSav() {
  $md = $_REQUEST['md']+0;
