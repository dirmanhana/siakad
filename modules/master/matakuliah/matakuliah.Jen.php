...
function TampilkanMenuJen() {
  global $mnux, $pref;
  echo "<p><a href='?mnux=$mnux&$pref=Jen&sub=JenAdd&md=1'>Tambah Jenis Mata Kuliah</a> |
  <a href='?mnux=$mnux&$pref=Jen&sub=CetakJenisMK'>Cetak</a></p>";
}
function DefJen() {
...
function TampilkanJenisMK() {
  ...
  echo "<tr><th class=ttl>Urutan</th>
  ...
  while ($w = _fetch_array($r)) {
    ...
    echo "<tr><td $c>$w[Urutan]</td>
    <td $c><a href='?mnux=$mnux&$pref=$_SESSION[$pref]&sub=JenAdd&md=0&jmkid=$w[JenisMKID]'><img src='img/edit.png' border=0>
    $w[Singkatan]</a></td>
    <td $c>$w[Nama]</td>
...
function JenAdd() {
  ...
  $_NA = ($w['NA'] == 'Y')? 'checked' : '';
  $_prd = GetOption2('prodi', "concat(ProdiID, ' - ', Nama)", 'ProdiID', $_SESSION['prodi'], '', 'ProdiID');
  // tampilkan formulir
  CheckFormScript("Singkatan,Nama,prodi");
  ...
  <tr><td class=inp1>Urutan Tampilan</td><td class=ul><input type=text name='Urutan' value='$w[Urutan]' size=5 maxlength=5></td></tr>
  <tr><td class=inp1>Singkatan</td><td class=ul><input type=text name='Singkatan' value='$w[Singkatan]' size=30 maxlength=20></td></tr>
  <tr><td class=inp1>Jenis Matakuliah</td><td class=ul><input type=text name='Nama' value='$w[Nama]' size=30 maxlength=50></td></tr>
  <tr><td class=inp1>Program Studi</td><td class=ul><select name='prodi'>$_prd</select></td></tr>
