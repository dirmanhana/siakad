...
function DaftarKelompokAsset() {
  ...
  $s = "select *
    ...
    order by kelompokID";
  $r = _query($s);
  echo "<p><a href='?mnux=kelompok.asset&gos=KelompokAssetEdt&md=1'>Tambahkan Kelompok Asset</a></p>";
  echo "<p><table class=box cellspacing=1 cellpadding=4>
    <tr>
	  <th class=ttl colspan=2 rowspan=2>#</th>
      <th class=ttl rowspan=2>Nama Kelompok</th>
  ...
  while ($w = _fetch_array($r)) {
    ...
    echo "<tr><td class=inp>$w[KelompokID] </td>
     <td class=ul><a href='?mnux=kelompok.asset&gos=KelompokAssetEdt&jid=$w[KelompokID]'><img src='img/edit.png'></a></td>
      <td $c>$w[Nama]</td>
...
function KelompokAssetEdt() {
  ...
  <input type=hidden name='mnux' value='kelompok.asset'>
  <input type=hidden name='gos' value='KelompokAssetSav'>
  <input type=hidden name='md' value='$md'>
  <tr><th class=ttl colspan=2>$jdl</th></tr>
  <tr><td class=inp>Ranking/Urutan</td>
    <td class=ul><input type=text name='KelompokID' value='$w[KelompokID]' size=5 maxlength=5></td></tr>
  ...
  <tr><td class=ul colspan=2><input type=submit name='Simpan' value='Simpan'>
    ...
    <input type=button name='Batal' value='Batal' onClick=\"location='?mnux=kelompok.asset'\"></td></tr>
  </form></table></p>";
}
...
function KelompokAssetSav() {
  ...
  if ($md == 0) {
    ...
    if (empty($ada)) {
      $s = "insert into kelompokasset (KelompokID, Nama, MasaKomersil, MasaFiskal, ProsentaseKomersil, ProsentaseFiskal, KodeID, LoginAdd, TangalAdd, NA)
        values ('$KelompokID', '$nm', '$manfaatkomersil', '$manfaatfiskal', '$prosentasekomersil', '$prosentasefiskal', '$KodeID', '$_SESSION[_Nama]', '$Actiondate', '$NA')";
      $r = _query($s);
      echo "<script>window.location = '?mnux=kelompok.asset'; </script>";
    }
    else {
...
// *** Main ***
TampilkanJudul("Daftar Kelompok $arrID[Nama]");
$gos();
