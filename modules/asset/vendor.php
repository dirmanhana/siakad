...
function DaftarVendor() {
  ...
  $s = "select *
    ...
    order by VendorID";
  $r = _query($s);
  echo "<p><a href='?mnux=vendor&gos=vendorEdt&md=1'>Tambahkan Vendor</a></p>";
  ...
  while ($w = _fetch_array($r)) {
    ...
    echo "<tr><td class=inp>$w[VendorID] </td>
     <td class=ul><a href='?mnux=vendor&gos=vendorEdt&jid=$w[VendorID]'><img src='img/edit.png'></a></td>
      <td $c>$w[Nama]</td>
      <td $c>$w[Alamat]</td>
      <td $c>$w[Telp]</td>
      <td $c>$w[Fax]</td>
...
function vendorEdt() {
  ...
  <input type=hidden name='mnux' value='vendor'>
  <input type=hidden name='gos' value='vendorSav'>
  <input type=hidden name='md' value='$md'>
  <tr><th class=ttl colspan=2>$jdl</th></tr>
  <tr><td class=inp>Ranking/Urutan</td>
    <td class=ul><input type=text name='VendorID' value='$w[VendorID]' size=5 maxlength=5></td></tr>
  ...
  <tr><td class=ul colspan=2><input type=submit name='Simpan' value='Simpan'>
    ...
    <input type=button name='Batal' value='Batal' onClick=\"location='?mnux=vendor'\"></td></tr>
  </form></table></p>";
}
...
function vendorSav() {
  ...
  if ($md == 0) {
    $s = "update vendor set VendorID='$vendorid', Alamat='$alamat', Telp='$telp', Fax='$fax', LoginEdit='$_SESSION[_Nama]', TglEdit='$Actiondate', NA='$NA'
    ...
    if (empty($ada)) {
      ...
      $r = _query($s);
      echo "<script>window.location = '?mnux=vendor'; </script>";
    }
    else {
