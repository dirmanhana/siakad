    ...
    if ($w['TahunID'] == $_SESSION['tahun']) {
      $ctk = ($ggl > 0) ? "<img src='img/check.gif' title='Tidak dapat dicetak karena ada KRS gagal.'>" : 
        "<a href='cetak/kss.cetak.php?tahun=$w[TahunID]&mhswid=$w[MhswID]&khsid=$w[KHSID]'><img src='img/printer.gif'></a>";
			//if ($w['KaliCetak'] == 0) {
				if ($thn['TglMundur'] >= date("Y-m-d")) {
					//if ($_SESSION['_LevelID'] != 1) {
						//$ctk = "<img src='img/check.gif' title='Tidak dapat dicetak karena sudah lewat batas waktu.'>";
					/*} else*/ $ctk = "<a href='?mnux=kss&gos=cekkss&tahun=$w[TahunID]&mhswid=$w[MhswID]&khsid=$w[KHSID]'><img src='img/printer.gif'></a>";
				}
			//}
		} else $ctk = '&nbsp;';
...
function cekkss() {
  ...
  if ($balance > 0) {
    ...
    $htg = GetaField('bipotmhsw', "MhswID='$mshwid' and TahunID='$tahun' and BIPOTNamaID", $acc['HutangNext'], "(Jumlah*Besar) - Dibayar")+0;
    //echo $htg; exit;
    if ($htg > 0) {
    ...
    $ggl = GetaField("krs", "NA='Y' and StatusKRSID<>'M' and KHSID", $khsid, "count(KRSID)")+0;
    if ($ggl > 0) {
...
function cetakkssgo() {
  echo "<script>
    new1 = window.open(\"cetak/kss.cetak.php?tahun=$_REQUEST[tahun]&khsid=$_REQUEST[khsid]&mhswid=$_REQUEST[mhswid]\",'mywindow','menubar=0,resizable=0,width=350,height=250');
  </script>";
  CetakKSS();
...
if (!empty($crmhswid)) {
  ...
  if (!empty($w)) {
    ...
    TampilkanHeaderBesar($w, 'kss', '', 0);
		$thn = GetFields('tahun', "ProgramID='$w[ProgramID]' and ProdiID='$w[ProdiID]' and TahunID", $tahun, '*');
    $gos();
  }
